...
6.  [Generate staking pool](#Generate-staking-pool)
7.  [Install Nearup](#Install-Nearup)
8.  [Launch validator node](#Launch-validator-node)
...
The wallet created by previous step is your main account ID, which can be used to generate your staking pool account. You can execute below instruction to generate your pool. 
```bash
near create-account <Your Pool ID> --masterAccount=<Your Master Account ID> 
example: near create-account pool.staketest.guildnet --masterAccount=staketest.guildnet
```
check your public key file of staking pool account.
```bash
ls ~/.near-credentials/guildnet/pool*
pool.staketest.guildnet
```
